...
bool CommunicationESP::connectWifi() {
  WiFi.disconnect(); 
  WiFi.mode(WIFI_STA);
...
void CommunicationESP::sendHtml() {
...
int CommunicationESP::getValue(String property, int factor) {
    String value = server.arg(property);
    return value.toFloat() * factor;
}

String CommunicationESP::buildRow(String property, int value, String step, int factor, int min, int max) {
